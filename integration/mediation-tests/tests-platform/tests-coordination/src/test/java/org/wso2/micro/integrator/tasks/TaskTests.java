...
public class TaskTests extends ESBIntegrationTest {
    ...
    void testTaskScheduling() throws Exception {
        ...
        boolean result = taskScheduledInNode1 ^ reader2.checkForLog(deploymentLog(TASK_1), CLUSTER_DEP_TIMEOUT);
        if (!result) {
    ...
    void testTaskExecution() throws Exception {
        ...
        String logCompleted = "completed-task-execution-cluster";
        if (taskScheduledInNode1) {
    ...
    void testTaskReSchedulingOfCompletedTask() throws Exception {
        ...
        Thread.sleep(15000); // for the task node to update completed status to task store
        if (taskScheduledInNode1) {
            reader2.clearLogs();
            node1.stopServer();
